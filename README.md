...
- 工具系统：内置 docker-sandbox 工具（`exec` / `exec_meta` / `browser_*`）
...
- `exec` 输出过大时，会由沙箱 API 返回预览和 artifact 文件路径（位于沙箱内 `/config/tool-artifacts`）。
- 需要二次读取大输出时，继续调用 `exec` 在沙箱中用 `rg/head/tail/sed` 等命令按需查询。
