...
- 工具系统：内置 docker-sandbox 工具（`bash` / `glob` / `read` / `grep` / `rg` / `browser_*`）
...
- `bash/glob/read/grep/rg` 的结果统一包含 `stdout` / `stderr`，优先读取 `preview`。
- 当输出被截断时，结果会包含对应 `path`（位于沙箱内 `/config/tool-artifacts/results`），可继续用 `bash` 配合 `rg/head/tail/sed` 按需查询。
